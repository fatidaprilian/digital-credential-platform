...
import { History, Loader2, ChevronsRight, ChevronsLeft, AlertCircle } from 'lucide-react';
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-[#EEEEEE]/10">
          ...
          <tbody className="divide-y divide-[#EEEEEE]/10">
            {isLoading ? (
              <tr><td colSpan={5} className="text-center py-20"><Loader2 className="animate-spin inline-block w-10 h-10 text-[#00ADB5]" /></td></tr>
            ) : history.length > 0 ? (
              paginatedHistory.map((log) => (
                <tr key={log.id} className="hover:bg-black/20 transition-colors duration-200">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-[#EEEEEE]/80">
                     <a href={`https://amoy.polygonscan.com/tx/${log.transactionHash}`} target="_blank" rel="noopener noreferrer" className="hover:underline hover:text-[#00ADB5] transition-colors">
              ...
              <tr>
                <td colSpan={5} className="text-center py-20 text-[#EEEEEE]/70">
